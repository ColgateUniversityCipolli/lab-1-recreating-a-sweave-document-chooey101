...
\end{knitrout}
\normalsize
The result is 40.9536588. Note that I did not type the result, but I used the command.
\subsection{Plotting}
...
xtable package (Dahl et al., 2019), which we can reference
using the label VADeaths.tab.
\scriptsize
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{library}\hldef{(xtable)}
\hldef{sleep.table}\hlkwb{<-}\hlkwd{xtable}\hldef{(VADeaths,} \hlkwc{label} \hldef{=} \hlsng{"VADeaths.tab"}\hldef{,} \hlkwc{caption} \hldef{=} \hlsng{"Death Rates per 1000 in Virginia (1940)."}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\normalsize
